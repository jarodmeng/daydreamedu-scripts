...
Optional: basic_meanings reading review
    → review_extra_basic_meanings_pinyin.py decisions artifact
    → Removes or corrects extra 基本字义解释 读音 not present in top-level 拼音
    → Updates extracted_characters_hwxnet.json

Optional: uncovered polyphonic reading review
...
Separately, `基本字义解释` is no longer treated as an untouchable raw extract. We now allow a small curated cleanup pass for `基本字义解释[].读音` when a reading appears only inside `basic_meanings` and not in the formal `拼音` list. This is mainly for archaic readings, malformed neutral-tone placeholders, and obvious typos (for example `mb` -> `ma`, `hàn` -> `hǎn` where appropriate). That review is tracked by a dedicated decisions artifact and then applied back into the JSON before DB sync.

Exception handling note: `嗯` is a special manually curated row. Unlike the general cleanup pass above, its `拼音` list and `基本字义解释` were filled in directly because the raw HWXNet-derived row had numbered pinyin placeholders, no structured `basic_meanings`, and no usable reading-aware content for downstream consumers.

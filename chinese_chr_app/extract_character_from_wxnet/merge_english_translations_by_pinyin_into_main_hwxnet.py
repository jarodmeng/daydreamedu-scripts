#!/usr/bin/env python3
"""
Merge the HWXNet 英文解释按拼音 transition field into extracted_characters_hwxnet.json.

Creates a descriptive backup of the main JSON in chinese_chr_app/data/backups/
before overwriting. The legacy 英文翻译 field is left unchanged.
"""
...
from build_english_translations_by_pinyin_transition import (
    DATA_DIR,
    FIELD_NAME,
    MAIN_JSON,
    REVIEWED_JSON,
...
def main() -> int:
    ...
    backup_path = BACKUP_DIR / f"extracted_characters_hwxnet.{ts}-english-glosses-by-pinyin-backup.json"
    ...
    if stats["polyphonic_units_missing_glosses"]:
        missing = stats["polyphonic_units_missing_glosses"]
        print(
            "Polyphonic units with empty gloss buckets: "
            + ", ".join(missing[:20])

#!/usr/bin/env python3
"""
Add common_phrases_by_pinyin column to hwxnet_characters and backfill from extracted_characters_hwxnet.json.

This script only adds/updates the common_phrases_by_pinyin column. It does not modify
any other column. Use --no-backup when you've already created a single backup table for
the whole rollout.
"""
...
ADD COLUMN IF NOT EXISTS common_phrases_by_pinyin jsonb;
"""
...
def _normalize_bucket(bucket):
    ...
    if not isinstance(pinyin, str) or not isinstance(phrases, list):
        return None
    ...
    for phrase in phrases:
        ...
        if not text or text in seen:
            continue
        seen.add(text)
        out_phrases.append(text)

    pinyin_text = pinyin.strip()
    if not pinyin_text:
        return None
    return {"Pinyin": pinyin_text, "Phrases": out_phrases}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Add common_phrases_by_pinyin to hwxnet_characters and backfill from extracted_characters_hwxnet.json."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.dry_run:
        print("Dry run: sample 常用词组按拼音 from JSON")
        ...
        print(f"Characters with non-empty 常用词组按拼音: {non_empty}")
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            conn.commit()
        print("Column common_phrases_by_pinyin added (or already exists).")
        ...
        for i in range(0, len(rows), batch_size):
            ...
            with conn.cursor() as cur:
                for r in batch:
                    ...
                    cur.execute(
                        "UPDATE hwxnet_characters SET common_phrases_by_pinyin = %s::jsonb WHERE character = %s AND zibiao_index = %s",
        ...
        print(f"Backfilled common_phrases_by_pinyin for {updated} rows.")
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE common_phrases_by_pinyin IS NOT NULL
                  AND jsonb_array_length(common_phrases_by_pinyin) > 0
                """
            )
            non_empty_count = cur.fetchone()[0]
        print(f"Rows with non-empty common_phrases_by_pinyin: {non_empty_count}")
    finally:
        conn.close()

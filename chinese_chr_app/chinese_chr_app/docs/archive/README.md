# Archive

This folder holds **pre-refactor** documentation moved here during the modular docs transition (Phase 1). Use it to double-check the transition; the canonical docs are now:

- [VISION.md](../VISION.md) — product strategy
- [ARCHITECTURE.md](../ARCHITECTURE.md) — technical specs
- [CHANGELOG.md](../CHANGELOG.md) — version history

**Contents:**

- **CONTENT_MAPPING_MODULAR_DOCS.md** — Phase 1 refactor plan (mapping, ground truth, versioning backfill). Status: Implemented; date: 2025-02-25.
- **Product_Requirements_Doc.md** — Original PRD (overview, milestones, learning vision).
- **plans/** — MVP1 and MVP2 session plans.
- **proposals/** — Implemented and draft proposals (scoring, profile categories, queue, etc.).
- **research/** — Learning-functions research and algorithm design notes.

Do not delete without reviewing; these are the source materials for VISION and CHANGELOG.

...
**What it is:** A local Python library (`PdfFileManager`) that keeps **`pdf_files`**, relations, **`file_groups`**, and **`book_answer_mappings`** aligned with on-disk paths. Every mutation is recorded in **`operation_log`**. The package tracks exams, exercises, books, activities, compositions, notes, and templates (with optional completions), including **first-class book unit → answer-page ranges** (not stored in `pdf_files.metadata`).
...
- **Composition folders:** Under `.../Composition/...`, scan sets `doc_type='composition'` and `metadata.content_folder=Composition`. Student writing samples live here — distinct from generic **`Activity/`** topic work. g_root Power Pack situational-writing **book** units stay under `Book/` as `doc_type='book'` ([proposal 18](../../../ai_study_buddy/pdf_file_manager/docs/proposals/18-composition-doc-type.md)).

...
This document defines 7 canonical question types for Primary Chinese Paper 2 (试卷二). They inform how sections are labelled for marking and learning analytics. They do not necessarily correspond 1:1 with the paper's official section hierarchy.

### Canonical type vs printed section title

School worksheets often print section names that are **finer-grained** than these seven canonical types (for example **词语搭配**, **辨字测验**, **词语选择**). When reducing a paper to structured data, use the **seven canonical values** below as **`question_type`** so classifications stay comparable across worksheets; preserve the paper's own headings, marks, and page ranges in companion fields whenever useful.

- **`question_type`** is always one of the **seven canonical values** below—the primary label downstream marking and tooling should honour (including stems and answer-booklet behaviour).
- **`printed_section_title`** (optional metadata) carries the **verbatim printed heading** when it helps readers or reporting—for example **`question_type`**: **`语文应用`** with **`printed_section_title`**: **`词语搭配`** when the layout is still independent MCQ-style language work but the paper labels a collocation block separately.
- **`section_total_marks`** (optional metadata) is the section’s **total marks** only when the value read from the paper is trustworthy **and** applies to that canonical section alone—for example separate printed totals for **阅读理解二A MCQ** vs **阅读理解二A 写作**; omit when uncertain or when one printed total clearly bundles split parts without a defensible split.

Omit **`printed_section_title`** or set it to **`""`** when the printed title does not add information beyond the heading already captured for that block, or when it matches the canonical type name.
...
This is a distinct canonical question type even though it shares the same parent section/group header and passage stem as "阅读理解二A MCQ".

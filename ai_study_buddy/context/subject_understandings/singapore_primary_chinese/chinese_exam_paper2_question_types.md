...
School worksheets often print section names that are **finer-grained** than these seven canonical types (for example **词语搭配**, **辨字测验**, **词语选择**). Structured outputs (for example a **`paper2_question_sections.json`** artifact from automation) should still use the **seven canonical values** below as **`question_type`** so routing stays stable; auxiliary columns or nested fields can carry the paper’s own wording, marks, and page ranges. This document does **not** pin a JSON Schema version—machine validation belongs with the detector or pipeline that emits those files.
...
- **`printed_section_title`** (optional, when your pipeline records it) holds the **verbatim printed heading** when it helps humans or analytics—for example **`question_type`**: **`语文应用`** with **`printed_section_title`**: **`词语搭配`** when the layout is still independent MCQ-style language work but the paper labels a collocation block separately.
- **`section_total_marks`** (optional, when your pipeline records it) is the section’s **total marks** only when the value read from the paper is trustworthy **and** applies to that canonical section alone—for example separate printed totals for **阅读理解二A MCQ** vs **阅读理解二A 写作**; omit when uncertain or when one printed total clearly bundles split parts without a defensible split.

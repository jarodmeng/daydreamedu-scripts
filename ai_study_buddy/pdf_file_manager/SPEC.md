...
**Book-aware behavior:** For paths under `.../Book/<book name>/...`, scan applies path inference so files are registered with `doc_type='book'`, infers `metadata.unit` from filename where possible, and syncs a `group_type='book'` file group for that folder using `<book name>` as the group label. Book-group membership is reconciled to canonical **general-scope template** mains for that folder. Student-mirror Book paths are skipped for group mutation; student files remain discoverable via template relations instead of duplicate group membership.

...
- Filter controls do not refresh results on change; click **Filter** to run `/api/inventory` and update the URL. **Sort** applies immediately on change (also via **Filter**); server orders by `recent` with unified recency (`completion_date` first, fallback to `registry_added_at`, newest first across both; unregistered last) or `name` (display name A–Z). Each apply refreshes dropdown options from the contextual slice (`root_ids`, `subjects`, `grades`, `doc_types`, `student_ids`, `book_names`, workflow visibility) in response `meta`. **Reset** restores defaults and reloads.
...
- Sort label **Completed (recent)** for `sort=recent` (requires `files` v0.3.7+ for unified recency fallback).

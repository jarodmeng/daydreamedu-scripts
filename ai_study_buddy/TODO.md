...
- [ ] **P1-5** · 2026-05-28 16:57 SGT: **Proposal 20 follow-through (medium + long term)** — after Phase A guardrails, implement remaining ID-first work from [`marking/docs/proposal/20-id-first-artifact-identity-and-path-decoupling.md`](marking/docs/proposal/20-id-first-artifact-identity-and-path-decoupling.md): (1) medium-term `marking_run_id` introduction and review/amendment ID-first re-keying with legacy fallback telemetry, and (2) long-term full normalization/deprecation of stem-identity assumptions with backfill + reconciler hardening.

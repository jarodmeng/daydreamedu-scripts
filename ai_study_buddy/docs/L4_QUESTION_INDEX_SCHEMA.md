...
| `skill_tags` | array of strings | AI-proposed skill/concept tags (often slug-style for retrieval/filtering). For multi-part questions, the union across sub-parts. **Not** the same contract as `marking_result.v1.x` `question_results[].skill_tags` — GoodNotes marking uses syllabus paths for math/science and empty tags for English/Chinese per [L4_MARKING_RESULT_ARTIFACT](./L4_MARKING_RESULT_ARTIFACT.md) |
...
| `skill_tags` | array of strings | Skill/concept tags specific to this sub-part (same style as parent `questions[].skill_tags`; independent of marking-result taxonomy) |
...
The marking pipeline's `resolve_marking_context()` (in `ai_study_buddy/marking/core/context_resolver.py`) currently returns file-level context: attempt file, template file, answer file, answer page range, and question selection.
...
- `sub_parts`: array (empty if single-part), each with:
  ...
  - `skill_tags`: skill/concept tags specific to this sub-part (index / extraction style; same contract as `questions[].skill_tags` in the question schema **Semantic fields** table)
...
- `skill_tags`: skill/concept tags (union of sub-part tags for multi-part); same index-oriented style as above — not the same as `marking_result.v1.x` syllabus paths
...
| `skill_tags` | array of strings | Pedagogical tags. Some DB sketches assumed a verbatim copy from `questions[].skill_tags`; **file-canonical `marking_result.v1.x`** instead follows [L4_MARKING_RESULT_ARTIFACT](./L4_MARKING_RESULT_ARTIFACT.md) (syllabus paths for math/science; `[]` for English/Chinese for now). |
...
- `skill_tags`: index-linked flows may seed from `questions[].skill_tags`; GoodNotes marking writes should replace with syllabus-aligned paths (or `[]`) per [L4_MARKING_RESULT_ARTIFACT](./L4_MARKING_RESULT_ARTIFACT.md), not necessarily the index’s slug style

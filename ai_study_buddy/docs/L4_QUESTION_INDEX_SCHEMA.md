...
| `skill_tags` | array of strings | AI-proposed skill/concept tags (often slug-style for retrieval/filtering). For multi-part questions, the union across sub-parts. **Not** the same contract as `marking_result.v1` `question_results[].skill_tags` — GoodNotes marking uses syllabus paths for math/science and empty tags for English/Chinese per [L4_MARKING_RESULT_ARTIFACT](./L4_MARKING_RESULT_ARTIFACT.md) |
...
- `skill_tags`: skill/concept tags (union of sub-part tags for multi-part); same index-oriented style as above — not the same as `marking_result.v1` syllabus paths
...
| `skill_tags` | array of strings | Pedagogical tags. Some DB sketches assumed a verbatim copy from `questions[].skill_tags`; **file-canonical `marking_result.v1`** instead follows [L4_MARKING_RESULT_ARTIFACT](./L4_MARKING_RESULT_ARTIFACT.md) (syllabus paths for math/science; `[]` for English/Chinese for now). |

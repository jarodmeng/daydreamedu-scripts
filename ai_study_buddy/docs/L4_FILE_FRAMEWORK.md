...
2. The following second-level attributes apply to both template and completion registered files.
   ...
   - "type" refers to the content type of the registered file. Template registered files should only have 4 distinct main types: "exam", "exercise", "book", and "activity". Completion registered files also have those 4 types with 2 additional types: "composition" and "note". If we tighten the definition of "completion registered file" to include only the main types, then template and completion registered files have the same "type" enums, but some of the on-disk student-scoped files are no longer categorized as "completion registered files". This attribute appears in a registered file's path as a folder name too.
...
- **Default filter:** `doc_type` not in `activity`, `note`, or `composition`; paths with a `Not completed` segment (case-insensitive) are omitted — same WIP convention as GoodNotes leaf-registry reports. The gap check applies only to registration-ready completions that require a template link (`exam`, `exercise`, `book`). **Composition** completions are standalone writing samples and never require `completed_from` (proposal 18). Pass `--include-activity-note` to include activity and note in the gap table; pass `--include-not-completed` to include WIP `Not completed` paths; composition remains excluded.

...
2. **Index source:** every direct `*.pdf` under each PDF **leaf folder** for:
   ...
   - `list_goodnotes_leaf_folders_under_root(goodnotes_root)` — default GoodNotes profile (same as [`goodnotes-leaf-registry-report`](../../.cursor/commands/goodnotes-leaf-registry-report.md): exclude `Not completed`, `Review`, `^x[A-Z].*$` segments, and root-as-leaf `.`).
